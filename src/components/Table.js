...
function createData(image, source, author, title, date, url) {
  return { image, source, author, title, date, url };
}
...
export default function EnhancedTable() {
  ...
  const [filterIsCanceled, setFilterIsCanceled] = React.useState(true);
  ...
  const handleSearch = (value) => {
    const item = news.articles
      .filter(
        (a) =>
          (a.content && a.content.includes(value.trim())) ||
          a.title.toLowerCase().includes(value.trim())
      )
      .map((article) =>
        createData(
          article.urlToImage,
